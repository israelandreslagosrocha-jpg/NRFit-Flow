...
-- Naty Entrenadora - Máquina de Estados, Versionado de Snapshots e Idempotencia
-- ==============================================================================
...
ALTER TABLE public.memberships
    ...
    ADD COLUMN IF NOT EXISTS last_gateway_snapshot_observed_at TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS idx_memberships_last_snapshot_observed_at 
    ON public.memberships(last_gateway_snapshot_observed_at);
...
-- Provee serialización atómica (FOR UPDATE), protección contra snapshots desfasados
-- u obsoletos observados por la aplicación, detección de no-op para idempotencia
-- estricta y registro atómico en public.security_audit_events.
CREATE OR REPLACE FUNCTION public.apply_membership_transition_atomic(
    ...
    p_gateway_snapshot_observed_at TIMESTAMPTZ DEFAULT NULL,
...
BEGIN
    ...
    SELECT id, status, gateway_status, current_period_start, current_period_end, trial_ends_at, cancelled_at, last_gateway_snapshot_observed_at, trace_id
    INTO v_mem
    ...
    -- 2. Guarda contra snapshots obsoletos observados por la aplicación
    -- Si ya se procesó un snapshot con estampa observada posterior, descartar de forma segura
    IF p_gateway_snapshot_observed_at IS NOT NULL 
       AND v_mem.last_gateway_snapshot_observed_at IS NOT NULL 
       AND p_gateway_snapshot_observed_at < v_mem.last_gateway_snapshot_observed_at THEN
        RETURN jsonb_build_object(
            'success', true,
            ...
            'last_gateway_snapshot_observed_at', v_mem.last_gateway_snapshot_observed_at,
            'attempted_snapshot_observed_at', p_gateway_snapshot_observed_at
        );
    END IF;
    ...
    IF v_mem.status = p_new_status
       ...
       AND (p_gateway_status IS NULL OR v_mem.gateway_status IS NOT DISTINCT FROM p_gateway_status) THEN
        ...
        -- Si hay una estampa más reciente del snapshot observado, refrescarla sin registrar auditoría de transición
        IF p_gateway_snapshot_observed_at IS NOT NULL 
           AND (v_mem.last_gateway_snapshot_observed_at IS NULL OR p_gateway_snapshot_observed_at > v_mem.last_gateway_snapshot_observed_at) THEN
            UPDATE public.memberships
            SET last_gateway_snapshot_observed_at = p_gateway_snapshot_observed_at,
                updated_at = now()
            WHERE id = p_membership_id;
        END IF;
    ...
    SET status = p_new_status,
        ...
        cancelled_at = CASE 
            ...
            ELSE cancelled_at 
        END,
        last_gateway_snapshot_observed_at = GREATEST(
            COALESCE(p_gateway_snapshot_observed_at, now()), 
            COALESCE(v_mem.last_gateway_snapshot_observed_at, '-infinity'::timestamptz)
        ),
        updated_at = now()
    WHERE id = p_membership_id;

    -- 5. Registrar evento de auditoría transaccional únicamente ante cambio real de estado
